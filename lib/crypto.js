...
class Point {
   ...
   *  HARDENED: rejects x-coordinates not on secp256k1 (invalid-curve attack defense). */
  static fromXOnly(xBytes) {
    const x = bytesToBigint(xBytes);
    if (x <= 0n || x >= P) throw new Error('Invalid pubkey: x out of field range');
    const y2 = mod(x * x * x + 7n);
    let y = modSqrt(y2);
    // Verify the candidate y actually satisfies y² ≡ x³+7 (mod P).
    // modSqrt always returns *something* — if y² ≢ y2, x is not on the curve.
    if (mod(y * y) !== y2) throw new Error('Invalid pubkey: point not on secp256k1');
    if (y % 2n !== 0n) y = mod(-y);
    return new Point(x, y);
  }

  /** Recover point from 33-byte compressed public key.
   *  HARDENED: rejects x-coordinates not on secp256k1. */
  static fromCompressed(bytes) {
    const prefix = bytes[0];
    if (prefix !== 0x02 && prefix !== 0x03) throw new Error('Invalid compressed prefix');
...
async function schnorrSign(msgBytes, privKeyBytes) {
  ...
  const sig = concatBytes(rx, bigintToBytes32(s));

  // Defense-in-depth: verify our own signature before returning it.
  // Catches glitches, fault attacks, RowHammer-style memory corruption,
  // and implementation bugs that would otherwise emit a bad signature.
  const ok = await schnorrVerify(msgBytes, sig, px);
  if (!ok) throw new Error('schnorrSign: self-verify failed — refusing to emit signature');

  return sig;
}

/** BIP-340 Schnorr verify. msg=32 bytes, sig=64 bytes, pubkey=32 bytes (x-only). */
async function schnorrVerify(msgBytes, sigBytes, pubXOnlyBytes) {
  if (sigBytes.length !== 64 || pubXOnlyBytes.length !== 32 || msgBytes.length !== 32) return false;
  try {
    const r = bytesToBigint(sigBytes.slice(0, 32));
    const s = bytesToBigint(sigBytes.slice(32, 64));
    if (r >= P || s >= N) return false;

    const P_pt = Point.fromXOnly(pubXOnlyBytes);  // throws if not on curve

    const eHash = await taggedHash('BIP0340/challenge', sigBytes.slice(0, 32), pubXOnlyBytes, msgBytes);
    const e     = mod(bytesToBigint(eHash), N);

    // R = sG - eP. We compute sG + (-e)P  =  sG + (N-e)P.
    const sG  = Point.G.mul(s);
    const neP = P_pt.mul(N - e);
    const R   = sG.add(neP);

    if (R.isZero()) return false;
    if (R.y % 2n !== 0n) return false;     // R.y must be even (BIP-340 has_even_y)
...
/** NIP-44 v2 padded length per spec (https://github.com/nostr-protocol/nips/blob/master/44.md).
 *  NOT just next-power-of-2 — uses chunk granularity = nextPower/8 once nextPower > 256. */
function calcPaddedLen(len) {
  if (len <= 32) return 32;
  // Equivalent to: 1 << (floor(log2(len-1)) + 1)
...
// ══════════════════════════════════════════════════════════════
// NIP-44 v2: Cache-friendly variants (operate on pre-derived conv key)
// ══════════════════════════════════════════════════════════════
async function nip44EncryptWithConvKey(convKey, plaintext) {
  const nonce = randomBytes(32);
  const { chachaKey, chachaNonce, hmacKey } = await nip44GetMessageKeys(convKey, nonce);
...
async function nip44DecryptWithConvKey(convKey, encryptedB64) {
  const payload = Uint8Array.from(atob(encryptedB64), c => c.charCodeAt(0));

  const version = payload[0];
  if (version !== 2) throw new Error(`NIP-44: unknown version ${version}`);
  if (payload.length < 1 + 32 + 1 + 32) throw new Error('NIP-44: payload too short');
...
export {
  ...
  // HKDF (exposed for self-tests)
  hkdfExtract, hkdfExpand,
  // Curve (exposed for self-tests)
  Point, P, N, Gx, Gy,
  // Keys
  getPublicKey, generatePrivKey, schnorrSign, schnorrVerify, getEventHash,
  // Bech32
  nsecToHex, hexToNsec, hexToNpub, npubToHex, bech32Decode,
  // NIP-04
  nip04Encrypt, nip04Decrypt, ecdhSharedX,
  // NIP-44
  nip44Encrypt, nip44Decrypt,
  nip44GetConversationKey, nip44GetMessageKeys,
  nip44EncryptWithConvKey, nip44DecryptWithConvKey,
  nip44Pad, nip44Unpad, calcPaddedLen,
  // ChaCha20 (exposed for self-tests)
  chacha20
};
